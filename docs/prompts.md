...
These are prompts you paste into Claude Code to kick off common workflows.

**Tip:** Use `/compact` frequently in long sessions to summarize the conversation and free up context window without losing progress.
...
Then propose the next single smallest task to implement (max 10 lines).
...
```
Refactor only for readability and consistency with the Architecture section of CLAUDE.md.
...
## 6) Context reset prompt (use when context gets large)
...
```
Read docs/outline.md and docs/plan.md. What's left to implement?
Propose the next single smallest task.
```
